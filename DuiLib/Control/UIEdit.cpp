...
namespace DuiLib
{
	...
	CEditWnd::CEditWnd() : m_pOwner(NULL), m_hBkBrush(NULL), m_bInit(FALSE), m_bDrawCaret(FALSE), m_bHexOnly(FALSE),m_bNumbOnly(FALSE)
	{
	}
	...
	void CEditWnd::Init(CEditUI* pOwner)
	{
		...
		{
			uStyle |= (ES_MULTILINE  | WS_VSCROLL);
			uStyle &= ~ES_AUTOHSCROLL;
		}
		else if (uTextStyle & ES_NUMBER)
		{
			m_bNumbOnly = TRUE;
		}

		m_bHexOnly = m_pOwner->IsHexNumbOnly();
	...
	RECT CEditWnd::CalPos()
	{
		...
		if( (uTextStyle & DT_SINGLELINE) &&  lEditHeight < rcPos.GetHeight() ) {
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		}else if (uMsg == WM_CHAR)
		{
			if (m_bHexOnly )
			{
				if ((wParam < 0x30 || wParam>0x39) && (wParam< 'a' || wParam >'f') && (wParam <'A' || wParam >'F') && wParam!=VK_BACK)
				{
					return -1;
	...
	CEditUI::CEditUI() : m_pEditWnd(NULL), m_uMaxChar(255), m_bReadOnly(FALSE), 
		m_bPasswordMode(FALSE), m_cPasswordChar(_T('*')), m_bAutoSelAll(FALSE), m_uButtonState(0), 
		m_dwEditbkColor(0xFFFFFFFF), m_iWindowStyls(0),m_dwTipTextColor(0), m_bHexOnly(FALSE)
	{
	...
	{
		if (m_pEditWnd) return m_pEditWnd->GetHWND();
		return NULL;
	}
	...
	{
		m_sText = pstrText;
		if( m_pEditWnd != NULL ) Edit_SetText(*m_pEditWnd, m_sText);
		Invalidate();
	}
	...
	void CEditUI::SetReadOnly(BOOL bReadOnly)
	{
		...
		if( m_pEditWnd != NULL ) Edit_SetReadOnly(*m_pEditWnd, m_bReadOnly);
		Invalidate();
	}
	...
	void CEditUI::SetHexNumbOnly(BOOL bHexNumbOnly)
	{

		m_bHexOnly = bHexNumbOnly;
	...
	BOOL CEditUI::IsHexNumbOnly() const
	{
		return m_bHexOnly;
	}
	...
	{
		return m_uTextStyle&ES_NUMBER ? TRUE:FALSE;
	}
	...
	{
		if (m_pEditWnd == NULL) return NULL;
		return m_pEditWnd->GetHWND();
	}
	...
	void CEditUI::SetPasswordChar(TCHAR cPasswordChar)
	{
		...
		if( m_pEditWnd != NULL ) Edit_SetPasswordChar(*m_pEditWnd, m_cPasswordChar);
		Invalidate();
	}
	...
	{
		if( m_pEditWnd != NULL ) Edit_SetSel(*m_pEditWnd, nStartChar,nEndChar);
	}
	...
	{
		if( m_pEditWnd != NULL ) Edit_ReplaceSel(*m_pEditWnd, lpszReplace);
	}
	...
	void CEditUI::SetPos(RECT rc, BOOL bNeedInvalidate)
	{
		...
		if( m_pEditWnd != NULL ) {
            ...
            if (::IsRectEmpty(&rcPos)) ::ShowWindow(m_pEditWnd->GetHWND(), SW_HIDE);
            else {
	...
	void CEditUI::SetVisible(BOOL bVisible)
	{
		...
		if( !IsVisible() && m_pEditWnd != NULL ) m_pManager->SetFocus(NULL);
	}
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		if( m_sText.IsEmpty() && m_sTipText.IsEmpty() ) return;
	...
	void CEditUI::SetTipText(LPCTSTR strVal)
	{
		if (m_sTipText!=strVal)
		{
			m_sTipText = strVal;
			Invalidate();
		}
	...
	void CEditUI::SetTipTextColor(DWORD dwColor)
	{
		if (m_dwTextColor!=dwColor)
		{
			m_dwTipTextColor = dwColor;
			Invalidate();
		}
	}
